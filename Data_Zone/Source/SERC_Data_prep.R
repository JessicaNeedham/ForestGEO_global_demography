...
site.name <- 'SERC'  # CTFS site name used in r table 
...
for (i in 1:n.census){
  cnsdata[[i]] <- cns[[i]][,c("TreeID", "Tag", 'StemID', "StemTag", "Mnemonic", 
                              "Quadrat", "PX", "PY", "DBH", "HOM", "Status", 
                              "Date", 'Codes')]
}

# CODES and STATUS
lapply(cnsdata, function(x) unique(x$Codes))
lapply(cnsdata, function(x) unique(x$Status))

# refactor status so it is binary
new.factors <- c('A', 'A', 'D', 'D', 'D')
names(new.factors) <- c('alive', 'broken below',
                        'dead', 'missing', 'stem dead')
for(i in 1:n.census){ 
  # if status is stem dead then make DBH NA
  cnsdata[[i]]$DBH <- ifelse(cnsdata[[i]]$Status == 'stem dead', 
                             NA, cnsdata[[i]]$DBH)
  # then refactor codes 
  cnsdata[[i]][ ,grep('Status', colnames(cnsdata[[i]]))] <- 
    revalue(cnsdata[[i]][ ,grep('Status', colnames(cnsdata[[i]]))],
            replace = new.factors)
  # if the site has 0 in dbh column for dead trees make them NAs
  cnsdata[[i]][which(cnsdata[[i]]$DBH == 0), 
               grep('DBH', colnames(cnsdata[[i]]))] <- NA
  # if DBH is NA then make HOM also NA
  cnsdata[[i]][which(is.na(cnsdata[[i]]$DBH)), 
               grep('HOM', colnames(cnsdata[[i]]))] <- NA
  # and convert the dbh to mm (if not already)
...
for(i in 1:n.census){
  colnames(cnsdata[[i]])[9:ncol(cnsdata[[i]])] <- 
    paste(colnames(cnsdata[[i]])[9:ncol(cnsdata[[i]])],".",i, sep="")
}

# match up the stems and make empty rows for the stems not in both
tmp <- cnsdata[[1]]

for(i in 2:n.census){
  tmp <- merge(tmp, cnsdata[[i]], 
               by = intersect(names(tmp)[1:8], names(cnsdata[[2]])[1:8]), 
               all = TRUE)  
  print(i)
}
...
colnames(all.sp) <- c('treeID', 'tag', 'StemID', 'stemTag', 'sp', 
                      'gx', 'gy', 'dbh.1', 
...
colnames(data.mat) <- c('treeID','tag', 'stemID', 'StemTag', 'sp.id', 'gx', 'gy', 'dbh.1', 'dbh.2', 
