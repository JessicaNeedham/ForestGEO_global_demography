...
site.name <- 'yasuni'  # CTFS site name used in r table 
...
for(i in censuses){ 
  load(paste('Data_Zone/Data/', site.name, '/', site.name, ".full",i,".rdata", sep=""))
}

# put them all in a list
cns <- vector('list', n.census)
for(i in 1:n.census){
  cns[[i]] = eval(as.name(paste( site.name, ".full",censuses[i], sep="")))
}

# load in the multi stem meta data
for(i in 1:n.census){
  load(paste('Data_Zone/Data/', site.name, '/', "mult", (i-1), ".rdata", sep=""))
}
# put them in a list
cns.multis <- vector('list', n.census)
for(i in 1:n.census){
  cns.multis[[i]] = eval(as.name(paste('mult', (i-1), sep="")))
}
...
cnsdata = list() 
for (i in 1:n.census){
  cnsdata[[i]] <- cns[[i]][ ,c('treeID', "tag", "stemID", "sp", "quadrat",
                              "gx", "gy", "dbh", "hom", "status", "date",
                              "ExactDate")]
}

# for each census add in the multi-stems - match tag of multistem to tag of main stem data - 
# give it all the same meta data and add to the main data
# NB some main stems have more than one multi stem
multi.stems <- vector('list', n.census)

for(i in 1:n.census){
  # make them both numeric
  cnsdata[[i]]$tag <- as.numeric(cnsdata[[i]]$tag)
  # get rid of stems from multistem that have no corresponding main stem
  cns.multis[[i]] <- cns.multis[[i]][which(cns.multis[[i]]$tag %in% 
                                             cnsdata[[i]]$tag == TRUE), ]
  
  multi.stems[[i]] <- cnsdata[[i]][sapply(cns.multis[[i]]$tag, 
                                          match, table = cnsdata[[i]]$tag), ]
  
# now change dbh of these rows to be the dbhs of the multistems
multi.stems[[i]]$dbh <- cns.multis[[i]][ ,grep('dbh', colnames(cns.multis[[i]]))]

# now bind them together
cnsdata[[i]] <- rbind(cnsdata[[i]], multi.stems[[i]])

# sort by treeID
cnsdata[[i]] <- cnsdata[[i]][order(cnsdata[[i]]$treeID), ]
}

head(cnsdata[[1]])
head(cnsdata[[2]])
length(which(cnsdata[[2]]$dbh > 500))
...
for(i in 2:n.census){
  tmp <- merge(tmp, cnsdata[[i]], 
               by = intersect(names(tmp)[c(1,2,4,5,6,7)], names(cnsdata[[2]])[c(1,2,4,5,6,7)]), 
               all = TRUE)  
  print(i)
}

length(which(tmp$dbh.2 > 500))
...
colnames(data.mat) <- c('treeID','tag', 'sp.id', 'gx', 'gy', 'dbh.1', 'dbh.2', 
...
resampled.dbhs <- correct.dbh(dbhs, 
                              times, 
                              give.up = give.up)
...
# corrected sizes
med.corrections <-  resampled.dbhs$dbh.stars
