...
site.name <- 'pasoh'  # CTFS site name used in r table 
censuses <- c(5,6)      # which censuses to use
...
# make a csv of weird dates for Yao
cens.six <- cnsdata[[2]]
eighties.ind <- which(str_sub(cens.six[ ,'ExactDate.2'], 1,4) == '1980')
# just the problem stems
eighties <- cens.six[eighties.ind, ]

# here we saved them and sent them to Yao - he was able to find
# the correct dates
#write.csv(eights, 'Data_Zone/Output/pasoh_date_stems.csv', 
 #         row.names = FALSE, quote = FALSE)

# so read in the corrected dates here
eighties.cor <- read.csv('Data_Zone/Data/pasoh/pasoh_date_stems_corrected.csv',
                       stringsAsFactors = FALSE)
cnsdata[[2]][eighties.ind, ] <- eighties.cor

# convert all the date columns into numerics using Exact date columns - 
# to be consistent with the new dates which had correct exact dates 
# but not corrected dates
for(ii in 1:length(cnsdata)){
  tmp <- cnsdata[[ii]]
  tmp[ ,grep('date', colnames(tmp))] <- as.numeric(ymd(tmp[ ,grep('ExactDate', colnames(tmp))]))
  cnsdata[[ii]] <- tmp
}
...
indv  <- cnsdata[[1]][,1:8]
temp2 <- lapply(cnsdata,"[", -c(1:8))
# bind dynamic data to static data
tmp <- cbind(indv, do.call(cbind ,(temp2)))
rm(temp2)
rm(indv)

# if date is NA make exact date NA too
# (some stems have '0000'00-00' in ExactDate.5)
for(ii in 1:n.census){
  tmp[ ,paste0('ExactDate.', ii)] <- ifelse(is.na(tmp[ ,paste0('date.', ii)]), 
                                            NA, tmp[ ,paste0('ExactDate.', ii)])
}
...
# first make sure all 0s in census 6 become NA
