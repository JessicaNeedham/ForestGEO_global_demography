...
site.name <- 'korup'  # CTFS site name used in r table 
...
# convert all dbhs to mm units and make 0s NA
# (they are not useful here for size dependent vital rates)
for(i in 1:n.census){ 
  cnsdata[[i]]$dbh <-cnsdata[[i]]$dbh * dbh.factor
  cnsdata[[i]]$dbh <- ifelse(cnsdata[[i]]$dbh == 0, NA, cnsdata[[i]]$dbh) 
} 
...
tmp$ExactDate.1 <- ifelse(nchar(tmp$ExactDate.1) < 10, NA, tmp$ExactDate.1)
tmp$ExactDate.2 <- ifelse(nchar(tmp$ExactDate.2) < 10, NA, tmp$ExactDate.2)

years <- tmp[ ,grep('Date', colnames(tmp))]
years <- apply(years, 2, 
               function(x) str_sub(x, 1,4))
years[ ,2] <- ifelse(years[ ,2] == '0000', NA, years[ ,2])
...
tmp$dbh.1 <- ifelse(tmp$status.1 == 'D', NA, tmp$dbh.1)
