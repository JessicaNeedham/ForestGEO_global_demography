...
for (i in censuses){ 
  load(paste('Data_Zone/Data/', site.name, '/', site.name,
             ".stem",i,".rdata", sep=""))
}
...
# in the survival code we will account for multi stems and only count death 
# of whole individual
...
S <- ifelse(is.na(size.mat[ ,2]) & !is.na(size.mat[ ,1]), 0, NA)
S <-  ifelse(!is.na(size.mat[ ,2]) & !is.na(size.mat[ ,1]), 1, S)
...
wsg <- tmp[match(all.sp[ ,'sp'], tmp[ ,'sp']) ,'wsg']
wsg.level <- tmp[match(all.sp[, 'sp'], tmp[ ,'sp']), 'wsglevel']
all.sp <- cbind(all.sp, wsg, wsg.level)

# for those with no info from the BCI species table fill in from global database
...
nones <- which(all.sp$wsg.level == 'none')
all.sp$wsg[nones] <- NA
all.sp$wsg.level[nones] <- NA
all.sp$wsg <- ifelse(is.na(all.sp$wsg), wsgs[match(all.sp$Latin, names(wsgs))], all.sp$wsg)
...
# remove new recruits - cannot be used in growth and survival
recruits <- which(is.na(size.mat[ ,1]))

if(length(recruits > 0)){
  tmp <- tmp[-recruits, ]
}
...
data.mat <- as.matrix(cbind(as.matrix(data.mat[ ,1:4]), 
                            sp.id, gx, gy, sizes, survival, incrs, times, wsg, wsg.level))
...
times.list <- lapply(times.list, function(x) matrix(x, ncol = ncol(times)))
...
out <- foreach(proc = 1:nproc)%dopar% {
  ...
  sampled <- unlist(resampled.dbhs$n.sampled)
...
sp.latins <- cbind(tmp$Latin[sps], tmp$sp[sps], tmp$wsg[sps], tmp$wsglevel[sps])

colnames(data.mat) <- c('treeID', 'stemID', 'tag', 'StemTag', 'sp.id', 'gx', 'gy', 
