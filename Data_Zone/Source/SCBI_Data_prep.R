...
site.name <- 'SCBI'  # CTFS site name used in r table 
...
##############################################################################
# this makes a list of census data
cns <- vector('list', n.census)

# load in all censuses
for (i in censuses){ 
  cns[[i]] <- read.csv(file = sprintf('Data_Zone/Data/%s/%s_stem_%d.csv',
                                      site.name, site.name, i),
                       header = TRUE)
}


# extract latin names and codes at this stage - trim down later to just
# the species that we fit demographic models to 
sp.latins <- unique(cns[[1]][ ,c('Latin', 'Mnemonic')])
sp.latins$Genus <- word(sp.latins[ ,'Latin'], 1,1)

save(sp.latins, file = sprintf('Data_Zone/Data/%s/%s.spptable.RData',
             site.name, site.name))

# only keep tags, species, quadrats, coordinates, height of measure, status, 
# date and exact date and code
cnsdata = list() 

for (i in 1:n.census){
  cnsdata[[i]] <- cns[[i]][,c("TreeID", "StemTag", "Mnemonic", 
                              "Quadrat", "PX", "PY", "DBH", "HOM", "Status", 
                              "Date", 'Codes')]
}

for(i in 1:n.census){ 
  # convert the dbh to mm (if not already) and change 0 DBH to NA
  cnsdata[[i]][,grep('DBH', colnames(cnsdata[[i]]))]<-  
    cnsdata[[i]][,grep('DBH', colnames(cnsdata[[i]]))]*dbh.factor
  cnsdata[[i]][ ,grep('DBH', colnames(cnsdata[[i]]))] <- 
    ifelse(cnsdata[[i]][ ,grep('DBH', colnames(cnsdata[[i]]))] == 0, 
           NA, cnsdata[[i]][ ,grep('DBH', colnames(cnsdata[[i]]))])
} 

# this appends the census number to time-dependent variables in each census
for(i in 1:n.census){
  colnames(cnsdata[[i]])[7:ncol(cnsdata[[i]])] <- 
    paste(colnames(cnsdata[[i]])[7:ncol(cnsdata[[i]])],".",i, sep="")
}
...
for(i in 2:n.census){
  tmp <- merge(tmp, cnsdata[[i]], 
               by = intersect(names(tmp)[1:6], names(cnsdata[[2]])[1:6]), 
               all = TRUE)  
  print(i)
}

# remove anything missing, stem dead or dead in census 1
# (if there is no DBH it cannot be used for size dependent vital rates)
tmp <- tmp[-(which(tmp$Status.1 == 'dead' | tmp$Status.1 == 'missing' |
                     tmp$Status.1 == 'stem dead')), ]

# refactor the status column to dead or alive  
# accordingly
new.factors <- c('A', 'A', 'D', 'D', 'D')
names(new.factors) <- c('alive', 'broken below', 'dead', 'missing', 'stem dead')
# nb if a stem is broken below but it has no dbh we have to call it dead
tmp$Status.1 <- revalue(tmp$Status.1, replace = new.factors)
tmp$Status.2 <- revalue(tmp$Status.2, replace = new.factors)

# remove any stems that are NA in both census
tmp <- tmp[-which(is.na(tmp$DBH.1) & is.na(tmp$DBH.2)), ]

# check that NAs in DBH have D in status (only census 2 because 
# NA in census 1 could be a new recruit)
tmp$Status.2 <- ifelse(is.na(tmp$DBH.2), 'D', tmp$Status.2)
...
dates <- apply(dates, 2, function(x) sapply(x, as.Date, format = '%d/%m/%Y'))
...
all.sp <- tmp[ ,!names(tmp) %in% c("Quadrat",
                                   paste0("HOM.", seq_along(1:n.census)),
                                   paste0("Codes.", seq_along(1:n.census)),
                                   paste0("Status.", seq_along(1:n.census)))]
# tidy the workspace
remove(cns)
remove(cnsdata)
remove(tmp)
colnames(all.sp) <- c('treeID', 'stemTag', 'sp', 'gx', 'gy', 'dbh.1', 
                      'date.1', 'dbh.2', 'date.2', 'surv.1')
...
sp.mat <- sp.latins
all.sp$Genus <- sp.mat[match(all.sp$sp, sp.mat$Mnemonic) ,'Genus']
all.sp$Latin <- sp.mat[match(all.sp$sp, sp.mat$Mnemonic), 'Latin']
...
sps <- match(sp.names, tmp$Mnemonic)
sp.latins <- tmp[sps, ]
