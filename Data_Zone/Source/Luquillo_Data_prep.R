...
site.name <- 'luquillo'  # CTFS site name used in r table 
censuses <- c(4,5)      # which censuses to use
...
tmp <- read.csv(sprintf('Data_Zone/Data/%s/lfdp_spp.csv', site.name), sep = '', header =TRUE)
tmp <- tmp[ ,1]
tmp <- do.call(rbind, sapply(tmp, str_split, ','))
rownames(tmp) <- NULL
colnames(tmp) <- c('sp', 'Family', 'Genus', 'Species', 'Life form')

tmp[ ,'Family'] <- simpleCap(tmp[ ,'Family'])
tmp[ ,'Genus'] <- simpleCap(tmp[ ,'Genus'])
tmp[ ,'Species'] <- tolower(tmp[ ,'Species'])
Latin <- paste(tmp[ ,'Genus'], tmp[ ,'Species'], sep = ' ')
tmp <- cbind(tmp, Latin)
sp.mat <- as.data.frame(tmp)
...
save(sp.mat, file = sprintf('Data_Zone/Data/%s/%s.spptable.rdata', site.name, site.name))
...
sp.latins <- sp.latins[-which(is.na(sp.latins[ ,2])), ]
