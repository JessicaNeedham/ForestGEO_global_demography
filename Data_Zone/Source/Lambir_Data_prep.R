...
site.name <- 'lambir'  # CTFS site name used in r table 
censuses <- c(3,4)      # which censuses to use
...
# Remove those with status = 'P' in census 1 - these are recruited later
# can't be used for growth or survival
#tmp <- tmp[-which(tmp$status.1 == 'P'), ]

# remove trees with NA for dbh in census 1 and 2
tmp <- tmp[-which(is.na(tmp$dbh.1) & is.na(tmp$dbh.2)), ]

# remove trees dead in census 1
tmp <- tmp[-which(tmp$status.1 == 'D'), ]

# Now for problem trees

# 1. if status 1 == A and status 2 is NA and dbh 2 is NA call it dead
tmp$status.2[which(tmp$status.1 == 'A' & is.na(tmp$dbh.2))] <- 'D'

# 2. status 1 is NA, status 2 is 'A', measurements in both dbh  1 and dbh 2 
# change status 1 to alive
tmp$status.1[which(!is.na(tmp$dbh.1) & !is.na(tmp$dbh.2) & tmp$status.2 == 'A')] <- 'A'

# 3. status 2 is 'D', change dbh2 to NA
tmp$dbh.2[which(tmp$status.2 == 'D')] <- NA

# 4. status is NA in both, dbh is value in 1 and NA in 2. remove. 
# probably died before census 1? (only 17 trees)
dead1 <- which(is.na(tmp$status.1) & is.na(tmp$status.2))
tmp <- tmp[-dead1, ]

# 5. status 1 is alive, status 2 is NA. dbh2 is a value. 
# remove? don't know whether this is alive or dead (only 46 trees)
notsure <- which(is.na(tmp$status.2) & !is.na(tmp$dbh.2))
tmp <- tmp[-notsure, ]

# 6 Gs in status 2 - call dead
gs <- which(tmp$status.2 == 'G')
tmp$dbh.2[gs] <- NA
tmp$status.2[gs] <- 'D'

# 7. remove stems with NA for status 1 and dbh 1 - 
# can't know if these were alive (90 trees)
# NOTE THIS IS NOT RECRUITS WHICH SHOULD HAVE STATUS P
# BUT LOOKING AT SIZES THEY COULD BE RECRUITS AS MOST ARE VERY SMALL
nasone <- which(is.na(tmp$status.1) & is.na(tmp$dbh.1))
tmp <- tmp[-nasone, ]

# survival based on status column
S <- ifelse(tmp$status.1 == 'A' & tmp$status.2 == 'A', 1, NA)
S <- ifelse(tmp$status.1 == 'A' & tmp$status.2 == 'D', 0, S)
...
data.mat$gx <- as.numeric(data.mat$gx)
data.mat$gy <- as.numeric(data.mat$gy)
...
############################
# remove recruits - not needed in growth survival analysis
length(which(is.na(data.mat$dbh.1)))
